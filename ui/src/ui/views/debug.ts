...
export function renderDebug(props: DebugProps) {
  ...
  return html`
    <section class="grid grid-cols-2">
      ...
      <div class="card">
        <div class="card-title">${t("debug.rpcTitle")}</div>
        <div class="card-sub">${t("debug.rpcSub")}</div>
        <div class="form-grid" style="margin-top: 16px;">
          <label class="field">
            <span>${t("debug.rpcMethod")}</span>
            <input
              .value=${props.callMethod}
              @input=${(e: Event) => props.onCallMethodChange((e.target as HTMLInputElement).value)}
              placeholder=${t("debug.presencePlaceholder")}
            />
          </label>
          <label class="field">
            <span>${t("debug.rpcParams")}</span>
            <textarea
        ...
        (evt) => html`
                  <div class="list-item">
                    <div class="list-main">
                      ...
                      <div class="list-sub">${formatTimeShort(evt.ts)}</div>
                    </div>
                    <div class="list-meta">
