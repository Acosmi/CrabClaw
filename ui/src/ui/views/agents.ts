...
function getToolSections() {
  return [
    ...
    },
    {
      ...
      tools: [
        { id: "memory_search", label: "memory_search", description: t("agents.tool.memory_search") },
        { id: "memory_get", label: "memory_get", description: t("agents.tool.memory_get") },
      ],
    },
    {
      id: "sessions",
      ...
      tools: [
        ...
        { id: "sessions_spawn", label: "sessions_spawn", description: t("agents.tool.sessions_spawn") },
        { id: "session_status", label: "session_status", description: t("agents.tool.session_status") },
      ],
    },
    {
      id: "ui",
    ...
    },
    {
      ...
      tools: [{ id: "message", label: "message", description: t("agents.tool.message") }],
    },
    {
      id: "automation",
...
function buildAgentContext(
  ...
  return {
    ...
    identityName,
    identityEmoji,
    skillsLabel: skillFilter ? t("agents.overview.selected", { count: skillCount ?? 0 }) : t("agents.overview.allSkills"),
...
function renderSubagentDetailPanel(agent: GatewayAgentRow, props: AgentsProps) {
  ...
  return html`
    ...
    ${agent.id === "argus-screen"
      ? html`
        <div class="callout info" style="margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
          <span style="font-size: 16px;">🍎</span>
          <span>该智能体目前仅支持 macOS 系统。Linux 支持将在下一个版本更新中提供。</span>
        </div>
        <div class="card">
...
function renderAgentHeader(
  ...
  const subtitle = agent.identity?.theme?.trim() || t("agents.agentRoutingSub");
      ...
      onModelFallbacksChange(
        ...
        <div class="row" style="justify-content: flex-end; gap: 8px;">
          ...
          <button
            ...
            @click=${onConfigSave}
          >
            ${configSaving ? t("agents.overview.saving") : t("common.save")}
          </button>
        </div>
      </div>
    </section>
...
function resolveChannelLabel(snapshot: ChannelsStatusSnapshot, id: string) {
  const meta = Array.isArray(snapshot.channelMeta) ? snapshot.channelMeta.find((entry) => entry.id === id) : undefined;
  if (meta?.label) {
    return meta.label;
  }
...
function resolveChannelEntries(snapshot: ChannelsStatusSnapshot | null): ChannelSummaryEntry[] {
  ...
  for (const entry of (Array.isArray(snapshot.channelMeta) ? snapshot.channelMeta : [])) {
    ids.add(entry.id);
  }
...
function renderAgentChannels(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(context, t("agents.context.overview"))}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
      ...
      : html`
        ...
        const status = summary.total
          ? t("agents.channels.connected", { n: summary.connected, total: summary.total })
          : t("agents.channels.noAccounts");
        const config = summary.configured
          ? t("agents.channels.configured", { n: summary.configured })
          : t("agents.channels.notConfigured");
        const enabled = summary.total ? t("agents.channels.enabled", { n: summary.enabled }) : t("agents.channels.disabled");
...
function renderAgentCron(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(context, t("agents.context.scheduling"))}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
                    ...
                    <div class="list-main">
                      ...
                      <div class="chip-row" style="margin-top: 6px;">
                        ...
                        <span class="chip ${job.enabled ? "chip-ok" : "chip-warn"}">
                          ${job.enabled ? t("agents.cron.cronEnabled") : t("agents.cron.cronDisabled")}
                        </span>
          ...
          : html`
                          <div class="agent-file-header">
                            ...
                            <div class="agent-file-actions">
                              ...
                              >
                                ${params.agentFileSaving ? t("agents.overview.saving") : t("common.save")}
                              </button>
                            </div>
                          </div>
                          ${activeEntry.missing
...
function renderAgentTools(params: {
  ...
  const TOOL_SECTIONS = getToolSections();
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">
            ${t("agents.tools.sub")}
            <span class="mono">${t("agents.tools.enabledCount", { n: enabledCount, total: toolIds.length })}</span>
          </div>
        </div>
        <div class="row" style="gap: 8px;">
          ...
          >
            ${params.configSaving ? t("agents.overview.saving") : t("common.save")}
          </button>
        </div>
      </div>
  ...
  return html`
    <div class="list-item agent-skill-row">
      ...
      : nothing
        ...
        ${missing.length > 0
      ? html`<div class="muted" style="margin-top: 6px;">${t("agents.skill.missing", { list: missing.join(", ") })}</div>`
      : nothing
    }
        ${reasons.length > 0
      ? html`<div class="muted" style="margin-top: 6px;">${t("agents.skill.reason", { list: reasons.join(", ") })}</div>`
      : nothing
    }
      </div>
      <div class="list-meta">
