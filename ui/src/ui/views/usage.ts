...
import { formatDurationCompact, formatDateTime } from "../format.ts";
      ...
      <div class="card usage-mosaic">
        <div class="usage-mosaic-header">
          <div>
            ...
            <div class="usage-mosaic-sub">${t("usage.estimatesHint")}</div>
          </div>
      ...
      <div class="usage-mosaic-header">
        <div>
          ...
          <div class="usage-mosaic-sub">
            ${t("usage.estimatedFromSpans", { tz: timeZone === "utc" ? t("usage.utc") : t("usage.local") })}
          </div>
        </div>
...
function renderFilterChips(
  ...
  const sessionsLabel = selectedSession
    ...
    : selectedSessions.length === 1
      ...
      : `${selectedSessions.length} ${t("usage.nSessions", { n: selectedSessions.length }).split(" ").slice(1).join(" ")}`;
  ...
  const daysLabel = selectedDays.length === 1 ? selectedDays[0] : t("usage.nDays", { n: selectedDays.length });
  const hoursLabel =
    selectedHours.length === 1 ? `${selectedHours[0]}:00` : t("usage.nHours", { n: selectedHours.length });
    ...
    <section class="card" style="margin-top: 16px;">
      ...
      <div class="usage-summary-grid">
        ...
        <div class="usage-summary-card">
          ...
          <div class="usage-summary-sub">${t("usage.toolsUsed", { n: aggregates.tools.uniqueTools })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usage.errors")}
          ...
          <div class="usage-summary-sub">${t("usage.toolResults", { n: aggregates.messages.toolResults })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usage.avgTokensMsg")}
            <span class="usage-summary-hint" title=${tokensHint}>?</span>
          </div>
          <div class="usage-summary-value">${formatTokens(avgTokens)}</div>
          <div class="usage-summary-sub">${t("usage.acrossMessages", { n: aggregates.messages.total || 0 })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usage.avgCostMsg")}
        ...
        <div class="usage-summary-card">
          ...
          <div class="usage-summary-sub">${t("usage.ofTotal", { total: totalSessions })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usage.throughput")}
    ...
    <div class="card sessions-card">
      <div class="sessions-card-header">
        ...
        <div class="sessions-card-count">
          ${totalSessions !== sessions.length ? t("usage.shownTotal", { n: sessions.length, total: totalSessions }) : t("usage.shown", { n: sessions.length })}
        </div>
      </div>
        ...
        : html`
          ...
          return html`
              ...
              void copySessionName(s);
                  ...
                  ${sessions.length > 50 ? html`<div class="muted" style="padding: 8px; text-align: center; font-size: 11px;">${t("usage.more", { n: sessions.length - 50 })}</div>` : nothing}
                </div>
              `
    }
      ${selectedCount > 1
      ? html`
              <div style="margin-top: 10px;">
                <div class="sessions-card-count">${t("usage.selected", { n: selectedCount })}</div>
...
function renderSessionSummary(session: UsageSessionEntry) {
  ...
  const formatTs = (ts?: number): string => (ts ? formatDateTime(ts) : "—");
  ...
  return html`
    ...
    <div class="session-summary-grid">
      ...
      <div class="session-summary-card">
        ...
        <div class="session-summary-meta">${usage.toolUsage?.uniqueTools ?? 0} ${t("usage.tools")}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usage.errors")}</div>
        <div class="session-summary-value">${usage.messageCounts?.errors ?? 0}</div>
        <div class="session-summary-meta">${t("usage.toolResults", { n: usage.messageCounts?.toolResults ?? 0 })}</div>
      </div>
      <div class="session-summary-card">
    ...
    <div class="card session-detail-panel">
      <div class="session-detail-header">
        ...
        <div class="session-detail-stats">
          ${usage
      ? html`
            <span><strong>${formatTokens(usage.totalTokens)}</strong> ${t("usage.tokens")}</span>
            <span><strong>${formatCost(usage.totalCost)}</strong></span>
          `
      : nothing
    }
        </div>
        <button class="session-close-btn" @click=${onClose} title="${t("usage.closeSession")}">×</button>
      </div>
...
function renderTimeSeriesCompact(
  ...
  if (loading) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.loading")}</div>
      </div>
    `;
  }
  if (!timeSeries || timeSeries.points.length < 2) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.noTimelineDataShort")}</div>
      </div>
    `;
  }
...
function renderContextPanel(
  ...
  if (usage && usage.totalTokens > 0) {
    ...
    if (inputTokens > 0) {
      contextPct = t("usage.contextOfInput", { pct: `${Math.min((totalContextTokens / inputTokens) * 100, 100).toFixed(0)}%` });
    }
  }
  ...
  return html`
    <div class="context-details-panel">
      <div class="context-breakdown-header">
        <div class="card-title" style="font-size: 13px;">${t("usage.contextWeight")}</div>
        ${hasMore
      ? html`<button class="context-expand-btn" @click=${onToggleExpanded}>
                ${showAll ? t("usage.collapseAll") : t("usage.expandAll")}
              </button>`
      : nothing
    }
      </div>
      <p class="context-weight-desc">${contextPct || t("usage.contextWeightDesc")}</p>
      <div class="context-stacked-bar">
        <div class="context-segment system" style="width: ${pct(systemTokens, totalContextTokens).toFixed(1)}%" title="${t("usage.systemApprox")}${formatTokens(systemTokens)}"></div>
        <div class="context-segment skills" style="width: ${pct(skillsTokens, totalContextTokens).toFixed(1)}%" title="${t("usage.skillsApprox")}${formatTokens(skillsTokens)}"></div>
        <div class="context-segment tools" style="width: ${pct(toolsTokens, totalContextTokens).toFixed(1)}%" title="${t("usage.toolsApprox")}${formatTokens(toolsTokens)}"></div>
        <div class="context-segment files" style="width: ${pct(filesTokens, totalContextTokens).toFixed(1)}%" title="${t("usage.filesApprox")}${formatTokens(filesTokens)}"></div>
      </div>
      <div class="context-legend">
        ...
        <span class="legend-item"><span class="legend-dot tools"></span>${t("usage.topTools").split(" ").pop()} ~${formatTokens(toolsTokens)}</span>
        <span class="legend-item"><span class="legend-dot files"></span>${t("usage.injectedFiles")} ~${formatTokens(filesTokens)}</span>
      </div>
      ...
      ? (() => {
        ...
        return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.topTools").split(" ").pop()} (${toolsList.length})</div>
                    <div class="context-breakdown-list">
                      ${toolsTop.map(
          (t) => html`
                          <div class="context-breakdown-item">
      ...
      ? (() => {
        ...
        return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.injectedFiles")} (${filesList.length})</div>
                    <div class="context-breakdown-list">
                      ${filesTop.map(
          (f) => html`
                          <div class="context-breakdown-item">
...
function renderSessionLogsCompact(
  ...
  if (loading) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">${t("usage.conversationLogs")}</div>
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.loading")}</div>
      </div>
    `;
  }
  if (!logs || logs.length === 0) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">${t("usage.conversationLogs")}</div>
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.noLogs")}</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="session-logs-compact">
      <div class="session-logs-header">
        <span>${t("usage.conversationLogs")} <span style="font-weight: normal; color: var(--text-muted);">(${displayedCount} ${t("usage.messages").toLowerCase()})</span></span>
      ...
      onFilterRolesChange(
        ...
        >
          <option value="user" ?selected=${roleSelected.has("user")}>${t("usage.user")}</option>
          <option value="assistant" ?selected=${roleSelected.has("assistant")}>${t("usage.assistant")}</option>
          <option value="tool" ?selected=${roleSelected.has("tool")}>${t("usage.tool")}</option>
      ...
      ? html`
                <div class="muted" style="padding: 12px">${t("usage.noLogs")}</div>
              `
      : nothing
    }
      </div>
...
export function renderUsage(props: UsageProps) {
        ...
        <div class="row" style="justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 12px;">
          <div style="flex: 1; min-width: 250px;">
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 2px;">
              <div class="card-title" style="margin: 0;">${t("usage.pageTitle")}</div>
              <span style="
                display: inline-flex;
                align-items: center;
                gap: 6px;
  ...
  const filteredSessions: UsageSessionEntry[] = queryResult.sessions;
  ...
  return html`
    ...
    <section class="card usage-header ${props.headerPinned ? "pinned" : ""}">
      ...
      ? html`
                ...
                <span class="usage-metric-badge">
                  ${displaySessionCount !== 1 ? t("usage.sessionCountPlural", { count: displaySessionCount }) : t("usage.sessionCount", { count: displaySessionCount })}
                </span>
              `
      : nothing
    }
          ...
          >
            ${props.headerPinned ? t("usage.unpin") : t("usage.pin")}
          </button>
          <details
