...
export type ChatItem =
  ...
  | { kind: "readonly-run"; key: string; run: ChatReadonlyRunState };
...
export type MessageContentItem = {
  type: "text" | "tool_call" | "tool_result" | "image" | "document" | "audio";
  text?: string;
  name?: string;
  args?: unknown;
  /** Multimodal source (base64 or URL reference) */
  source?: { type: string; media_type?: string; data?: string; url?: string };
  /** Original file name for document/audio attachments */
  fileName?: string;
  /** File size in bytes */
  fileSize?: number;
};
