...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "agents") {
    ...
    if (agentId) {
      void loadAgentIdentity(host as unknown as OpenAcosmiApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as OpenAcosmiApp, agentId);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as OpenAcosmiApp);
    await loadDevices(host as unknown as OpenAcosmiApp);
    await loadConfig(host as unknown as OpenAcosmiApp);
    await loadExecApprovals(host as unknown as OpenAcosmiApp);
  }
  if (host.tab === "memory") {
    const app = host as unknown as OpenAcosmiApp;
    if ((app as { memoryPanel?: string }).memoryPanel === "sessions") {
  ...
  if (host.tab === "debug") {
    await loadDebug(host as unknown as OpenAcosmiApp);
...
export function inferBasePath() {
  ...
  const configured = window.__OPENACOSMI_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OpenAcosmiApp, false),
    loadPresence(host as unknown as OpenAcosmiApp),
    loadSessions(host as unknown as OpenAcosmiApp),
    loadCronStatus(host as unknown as OpenAcosmiApp),
    loadDebug(host as unknown as OpenAcosmiApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OpenAcosmiApp, true),
    loadConfigSchema(host as unknown as OpenAcosmiApp),
    loadConfig(host as unknown as OpenAcosmiApp),
  ]);
}
