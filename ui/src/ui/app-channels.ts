...
export async function handleWhatsAppWait(host: OpenAcosmiApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: OpenAcosmiApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: OpenAcosmiApp): Promise<boolean> {
  // Ensure config is loaded first (provides configSnapshot.hash for optimistic concurrency)
  if (!host.configSnapshot?.hash) {
    await loadConfig(host);
  }
  await saveConfig(host);
  // Only reload if save succeeded — don't wipe user input on failure
  if (!host.lastError) {
    await loadConfig(host);
    await loadChannels(host, true);
    return true;
...
export async function handleChannelConfigReload(host: OpenAcosmiApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: OpenAcosmiApp): string {
...
export function handleNostrProfileCancel(host: OpenAcosmiApp) {
...
export function handleNostrProfileToggleAdvanced(host: OpenAcosmiApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: OpenAcosmiApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: OpenAcosmiApp) {
  const state = host.nostrProfileFormState;
