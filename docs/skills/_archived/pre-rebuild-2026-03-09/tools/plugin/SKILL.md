...
2. Or install an external plugin by adding its path:

```json5
{
  plugins: {
    load: { paths: ["~/.openacosmi/extensions/voice-call"] }
  }
}
```
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@openacosmi/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@openacosmi/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@openacosmi/zalouser`
- [Matrix](/channels/matrix) — `@openacosmi/matrix`
- [Nostr](/channels/nostr) — `@openacosmi/nostr`
- [Zalo](/channels/zalo) — `@openacosmi/zalo`
- [Microsoft Teams](/channels/msteams) — `@openacosmi/msteams`
...
- `<workspace>/.openacosmi/extensions/*.ts`
- `<workspace>/.openacosmi/extensions/*/index.ts`
...
Bundled plugins must be enabled explicitly via `plugins.entries.<id>.enabled: true`
in config. Installed plugins are enabled by default, but can be disabled the same way.

Each plugin must include a `openacosmi.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `openacosmi.extensions`:
...
Channel plugins can advertise onboarding metadata via `openacosmi.channel` and
install hints via `openacosmi.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@openacosmi/nextcloud-talk",
  "openacosmi": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@openacosmi/nextcloud-talk",
...
- `~/.openacosmi/mpm/catalog.json`
- `~/.openacosmi/plugins/catalog.json`

Or point `OPENACOSMI_PLUGIN_CATALOG_PATHS` (or `OPENACOSMI_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "openacosmi": { "channel": {...}, "install": {...} } } ] }`.
...
- Plugin config is validated using the JSON Schema embedded in
  `openacosmi.plugin.json` (`configSchema`).
...
也可直接编辑 `~/.openacosmi/openacosmi.json` 中的 `plugins.entries.<id>.enabled` 管理插件。
...
```
import { registerPluginHooksFromDir } from "openacosmi/plugin-sdk";
...
- Plugin-managed hooks show up with `plugin:<id>` 前缀。
- 通过启用/禁用插件本身来控制其附带的 hooks。
...
- Main package: `openacosmi` (this repo)
- Plugins: separate npm packages under `@openacosmi/*` (example: `@openacosmi/voice-call`)

Publishing contract:

- Plugin `package.json` must include `openacosmi.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- npm 插件安装流程: `npm pack` 下载 → 解压到 `~/.openacosmi/extensions/<id>/` → 在 config 中启用。
...
- RPC CLI: `voicecall.start`, `voicecall.status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `openacosmi.extensions` points at the built entrypoint (`dist/index.js`).
