...
Crab Claw（蟹爪） can use **Firecrawl** as a fallback extractor for `web_fetch`. It is a hosted
...
Crab Claw（蟹爪） always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
