---
name: synthetic
description: "Use Synthetic's Anthropic-compatible API in Crab Claw（蟹爪）"
---

# Synthetic

Synthetic exposes Anthropic-compatible endpoints. Crab Claw（蟹爪） registers it as the
...
```bash
crabclaw onboard --auth-choice synthetic-api-key
```
...
Note: Crab Claw（蟹爪）'s Anthropic client appends `/v1` to the base URL, so use
