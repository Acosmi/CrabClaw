...
- `gateway`/`node` approvals are controlled by `~/.openacosmi/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Crab Claw（蟹爪） prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
```bash
crabclaw config get agents.list
crabclaw config set agents.list[0].tools.exec.node "node-id-or-name"
```
